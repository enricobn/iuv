...
import org.iuv.shared.utils.fold
...
class OpenAPICommand : CliktCommand(name = "openAPI") {
    ...
    override fun run() {
        OpenAPIProperties.read().fold({ e -> println(e) }) { openAPIProperties ->
            println(openAPIProperties)
            with(openAPIProperties) {
    ...
    private fun getSwaggerFiles(folder: File): List<File> {
        val yamlFiles = folder.listFiles { file: File -> file.extension == "yaml" || file.extension == "json" }
                ?: emptyArray()
